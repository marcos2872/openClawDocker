 ...
 *
 * Carrega dinamicamente todas as skills de .pi/skills/ e permite alternar
...
const DEFAULT_AGENT = "agent-build";
...
export default function agentSwitcherExtension(pi: ExtensionAPI): void {
  ...
  async function showAgentSelector(ctx: ExtensionContext): Promise<void> {
    if (skills.length === 0) {
      ctx.ui.notify("Nenhuma skill encontrada em .pi/skills/", "warning");
      return;
    }
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    if (skills.length === 0) {
      ctx.ui.notify(
        "agent-switcher: nenhuma skill encontrada em .pi/skills/",
        "warning",
      );
      return;
    }
